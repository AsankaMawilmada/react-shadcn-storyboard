...
@import '@inoaspect/react-components/styles/radio-group.css';
...
`@source` scan. `InputField`, `DateSelect`, `Select`, and `RadioGroup`
are the exception — their styles are consolidated into named classes
(`.input-field`, `.date-select`, `.select-error`, `.radio-group`,
...
`src/components/RadioGroup/RadioGroup.css`). The build copies
