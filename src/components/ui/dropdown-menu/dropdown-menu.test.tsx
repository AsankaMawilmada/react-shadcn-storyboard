...
describe('DropdownMenu', () => {
  ...
  it('toggles the checkbox item state on click', async () => {
    ...
    render(<Default />)
    await user.click(screen.getByRole('button', { name: 'Open menu' }))
    const statusBarItem = await screen.findByRole('menuitemcheckbox', { name: 'Show status bar' })
    const activityLogItem = screen.getByRole('menuitemcheckbox', { name: 'Show activity log' })
    ...
    expect(screen.getByRole('menuitemcheckbox', { name: 'Show activity log' })).toHaveAttribute(
      'aria-checked',
      'true',
    )
  })
