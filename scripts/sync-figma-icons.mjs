...
async function generateOne(fileName) {
  ...
  // Only `variables.jsx.children` is used below (the root <svg>'s own
  // attributes are discarded) — IconBase supplies its own root <svg>, sized
  // and styled from the shared IconProps, so per-icon files only need to
  // contribute their unique path data.
  const code = await transform(
    cleaned,
    {
      ...
      template: (variables, { tpl }) => {
        const iconBaseElement = t.jsxElement(
          t.jsxOpeningElement(
            t.jsxIdentifier('IconBase'),
            [
              t.jsxAttribute(
                t.jsxIdentifier('viewBox'),
                t.stringLiteral(viewBox),
              ),
              t.jsxAttribute(
                t.jsxIdentifier('defaultStrokeWidth'),
                t.stringLiteral(defaultStrokeWidth),
              ),
              t.jsxSpreadAttribute(t.identifier('props')),
            ],
            false,
          ),
          t.jsxClosingElement(t.jsxIdentifier('IconBase')),
...
export function ${variables.componentName}(props: IconProps) {
  return (${iconBaseElement})
}
`;
      },
    },
